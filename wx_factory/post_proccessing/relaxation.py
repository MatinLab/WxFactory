from . import post_proccessor
from scipy.optimize import newton, brentq
from numpy.typing import NDArray
from common import Configuration
from geometry import Cartesian2D, DFROperators
from solvers import nonlin
from init import entropy_vars as ev
import math
import numpy as np

class RelaxationPostProcessor(post_proccessor.PostProcessor):
    def __init__(self, config: Configuration, geom: Cartesian2D, operators: DFROperators, Q_initial: NDArray, rhs):
        if type(geom) != Cartesian2D or geom is None:
            raise TypeError("The relaxation post processor works only with a 2D cartesian geometry")
        self.geom = geom
        self.xp = geom.device.xp
        self.rhs = rhs
        self.Q_prev = None
        self.Q_current = None
        self.operators = operators
        self.dt = None
        self.ehat = None
        self.gamma = 1.0
        

    def residual(self, Q_prev: NDArray, Q_current: NDArray, rconst: float) -> float:
        # Compute the residual between the two entropies,
        Q_relaxed = Q_prev + rconst * (Q_current - Q_prev)
        first = ev.global_entropy(Q_relaxed, self.geom, self.operators)
        second = ev.global_entropy(Q_prev,   self.geom, self.operators)
        current = first - second
        #print(f"first = {first} ---------- Second = {second}")
        return current
    
    def update(self, Q_before: NDArray, Q_after: NDArray, dt: float, f_before: NDArray, f_after: NDArray):
        self.Q_prev    = Q_before.copy()
        self.Q_current = Q_after          # note: process() mutates this in-place
        self.dt        = dt
        
        # Trapezoidal estimate of entropy production over the step
        rate_n   = self.entropy_production_rate(Q_before)
        rate_np1 = self.entropy_production_rate(Q_after)
        self.e_hat = 0.5 * (rate_n + rate_np1)

    def process(self):
        if self.Q_current is None or self.Q_prev is None:
            return
        self._apply_relaxation(self.Q_prev, self.Q_current)
    
    def residual_prime(self, Q_prev, Q_current, rconst):
        Q_relaxed = Q_prev + rconst * (Q_current - Q_prev)
        dQ = Q_current - Q_prev
        directional = ev.entropy_prime_function(Q_relaxed, dQ, self.geom, self.operators)
        return directional - self.dt * self.e_hat
    
    def entropy_production_rate(self, Q):
        """Spatially-integrated entropy production rate σ for the DG scheme at state Q."""
        # Trigger the RHS evaluation to populate internal arrays
        _ = self.rhs(Q)
        sigma = self.rhs.entropy_residual()
        print(f"[diag] sigma: min={float(self.xp.min(sigma)):.3e}  max={float(self.xp.max(sigma)):.3e}  "
          f"sum={float(self.xp.sum(sigma)):.3e}  sum|σ|={float(self.xp.sum(self.xp.abs(sigma))):.3e}")
        return float(self.rhs.device.xp.sum(sigma))

    def _apply_relaxation(self, Q_prev: NDArray, Q_current: NDArray):
    
        r_values = np.concatenate([
        np.linspace(0.0, 0.1, 6),       # near the spurious root
        np.linspace(0.2, 0.9, 8),       # middle
        np.linspace(0.95, 1.05, 11),    # near the expected root
        np.linspace(1.1, 2.0, 5),       # above 1
    ])
        print(f"[sweep]   r            R(r)              R'(r)")
        for r in r_values:
            R  = self.residual(Q_prev, Q_current, float(r))
            Rp = self.residual_prime(Q_prev, Q_current, float(r))
            print(f"[sweep]  {r:8.5f}    {R: .6e}    {Rp: .6e}")
        print("[sweep] ---- end sweep ----")
        

        r_at_0   = self.residual(Q_prev, Q_current, 0.0)
        r_at_1   = self.residual(Q_prev, Q_current, 1.0)
        rp_at_1  = self.residual_prime(Q_prev, Q_current, 1.0)
        print(f"[diag] R(0)={r_at_0:.3e}  R(1)={r_at_1:.3e}  R'(1)={rp_at_1:.3e}  e_hat={self.e_hat:.3e}  dt={self.dt:.3e}")

        def r_func(gamma):
            return self.residual(Q_prev, Q_current, float(gamma))
        
        def r_func_prime(gamma):
            return self.residual_prime(Q_prev, Q_current, float(gamma))

        
        result = newton(r_func, x0 = 0.95, x1 = 1.5, tol=1e-8, maxiter=200)
        gamma = float(result)

        print(f"-------gamma currently at {gamma} --------")

        Q_current[:] = Q_prev + gamma*(Q_current - Q_prev)

    